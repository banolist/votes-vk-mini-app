...
const UserProfile: Component = () => {
  ...
  const votePeopleMutation = useMutation(() => ({
    ...
    onSuccess: async () => {
      queryClient.invalidateQueries({ queryKey: ["expertProfile", id] });
    },
  }));
        ...
        <Match when={query.isFetched && query.data}>
          {(user) => (
            <>
              <Avatar
                avatar={user().avatar}
                class="size-full rounded-3xl max-w-xl self-center"
              />
              <Card>
                {/* statistic */}
                ...
                <Show when={user().expertProfile}>
                  {(profile) => (
                    <div class="flex flex-col gap-1.25">
                      <StatListField
                        text="Направления"
                        items={profile().communityDirections?.map(
              ...
              <Show when={me.data?.agreesTermsAt && user().expertProfile}>
                {(profile) => (
                  <>
                    ...
                    <Show
                      when={hasRights(me.data?.rights || 0, UserRights.Admin)}
                    >
...
const AssignTariffModal: Component<{ userID: string }> = (props) => {
  const [, { Form, Field }] = createForm<AssignTariffForm>({
